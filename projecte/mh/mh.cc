...
Tactic constructGreedyRandomizedSolution(){
  ...
  while (s.goal.empty() or int(s.def.size()) < N1 or int(s.mid.size()) < N2 or int(s.str.size()) < N3){
    ...
    if(s.price + x.price <= T and x.price <= J){
      s.add(x);
    }
    ...
    tie(pricePond, pointsPond) = updatePonderations(s);
  }
...
void mh(char const *output){
  ...
  // sort players by points and position
  sort(PLAYERS.begin(), PLAYERS.end(), sortByPos);

  int MAX_POINTS = -1;
  Tactic s;

  // definir teminating conditions els dos cops
