...
void greedy(Tactic& t, char const *output){
  ...
  // Sort the players based on ratio price/points
  sort(PLAYERS.begin(), PLAYERS.end(), compare);

  for(const Player& p : PLAYERS){

    // Selected player requirements
    ...
    else if(p.position == "def" and int(t.def.size()) < N1){
      t.def.push_back(p.name);
      t.points += p.points;
      t.price += p.price;
    }
    else if(p.position == "mig" and int(t.mid.size()) < N2){
      t.mid.push_back(p.name);
      t.points += p.points;
      t.price += p.price;
    }
    else if(p.position == "dav" and int(t.str.size()) < N3){
